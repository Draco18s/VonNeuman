...
namespace Assets.draco18s.util {
	public static class SpriteLoader {
		private static readonly int PIXELS_PER_UNIT = 100;
		...
		public static GameObject gameObjectForResource(Sprite sprite) {
		...
		public static GameObject gameObjectForResource(string texName, Rect size) {
			...
			{
				Texture2D t = Resources.Load<Texture2D>(texName);
				s = Sprite.Create(t, size, Vector2.zero, PIXELS_PER_UNIT, 0, SpriteMeshType.FullRect);
		...
		public static GameObject gameObjectForResource(Texture2D t, Rect size) {
			Sprite s = Sprite.Create(t, size, new Vector2(0.5f, 0.5f), PIXELS_PER_UNIT, 0, SpriteMeshType.FullRect);
			GameObject go = new GameObject();
			SpriteRenderer renderer = go.AddComponent<SpriteRenderer>();
			renderer.sprite = s;
			//renderer.material = spriteMat;
		...
		public static Sprite getSpriteForResource(string texName) {
			...
			if (s == null)
			{
				...
				s = Sprite.Create(t, size, new Vector2(0.5f, 0.5f), PIXELS_PER_UNIT, 0, SpriteMeshType.FullRect);
				allSprites.Add(texName, s);
			}
			return s;
		}
